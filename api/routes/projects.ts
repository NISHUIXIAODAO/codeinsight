...
router.post('/import', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const taskId = (taskInserted as any)?.id as string | undefined
...
router.post('/:id/parse', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const taskId = (taskInserted as any).id as string
    setTimeout(async () => {
      try {
        await supabase.from('tasks').update({ status: 'running' }).eq('id', taskId)
        const graph = buildRepoGraph(localPath, {})
        await supabase.from('parse_results').insert([{ project_id: projectId, ast_data: null, dependencies: graph as any }])
        await supabase.from('tasks').update({ status: 'completed' }).eq('id', taskId)
...
router.get('/:id/dependencies', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const deps = (data?.dependencies as any) || null
